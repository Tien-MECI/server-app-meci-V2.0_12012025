...
async function prepareYcvtData(auth, spreadsheetId, spreadsheetHcId) {
    ...
    try {
        ...
        const hasDataI = tableDataFrom7.some(item => item.row[7] && item.row[7].toString().trim() !== '');
        const hasDataJ = tableDataFrom7.some(item => item.row[8] && item.row[8].toString().trim() !== '');
